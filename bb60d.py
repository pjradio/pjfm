...
class BB60D:
    ...
    def measure_signal_power(self):
        """
        Measure signal power at current frequency.

        Temporarily reconfigures IQ streaming to measure power.
        ...
        try:
            ...
            # Configure for quick IQ capture
            # Use decimation of 128 for ~312 kHz sample rate
            decimation = 128
            bandwidth = 200000.0  # 200 kHz bandwidth for FM

            status = bbConfigureIQCenter(self.handle, c_double(current_freq))
            if status < 0:
                return -140.0
            ...
            # Purge old data and get fresh samples
            status = bbGetIQUnpacked(
                self.handle, iq_data, num_samples,
                None, 0, BB_TRUE,  # purge=True
            ...
            if status < 0:
                ...
                # IQ samples are already calibrated voltage
                # Power = mean(|IQ|^2), convert to dBm
                mean_power = np.mean(np.abs(iq_data) ** 2)
                if mean_power > 0:
                    # BB60D IQ is calibrated, this gives approximate dBm
                    # The correction factor may need adjustment
                    power_dbm = 10 * np.log10(mean_power) - 10.0
                else:
                    power_dbm = -140.0
            ...
            bbAbort(self.handle)
            if was_streaming == "iq":
                self.configure_iq_streaming(current_freq)
